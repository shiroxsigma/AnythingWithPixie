...
# =====================================================
# max_tool_calls 到達時の最終回答強制生成
# =====================================================
# フェーズ3 eval のパターンB（ツールは呼ぶが完遂できず、最終回答を出さないまま
# max_tool_calls に到達し無回答で終了する）対策。到達時に最後の1回だけ
# tools=None（ツール呼び出し不可）で node_plan を呼び、「ここまでの結果から
# 最終回答をまとめよ」という一時指示（動的suffixと同方式・履歴非汚染）で
# 回答を生成させる。生成失敗・空応答・tool_calls混入時は従来の
# exit_reason（max_tool_calls_reached・無回答）にフォールバックする。

#: True で上記の強制生成を有効化。False で従来動作（強制終了のみ・最終回答なし）。
FORCE_FINAL_ANSWER_ON_LIMIT: bool = True
